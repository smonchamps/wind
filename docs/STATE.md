...
🔧 **Next subject**: **the beta feedback** (wave 1 open since 2026-08-31, five invitations out — a tester report goes first, in `/field`, on `main`; follow-up with the silent ones was due 2026-09-03). Also awaited: the GitHub support answer (purge of the dangling objects, ticket of 2026-09-02 — `gh api repos/smonchamps/wind/commits/051bb01…` ⇒ 404 is the check), the S4 measure (one week from 2026-08-30), the SAC net (an update refused for real), the D-50 Microsoft-refresh confirmation (90 days). With the audit closed, the product PLAN's path is the beta itself.
