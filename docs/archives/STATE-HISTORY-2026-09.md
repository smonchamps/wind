> **Historical record — frozen** (extracted from `docs/STATE.md` on
> 2026-09-07; audit Lot 6 E16d, finding C07 "STATE retains history").
> The living state is [`docs/STATE.md`](../STATE.md); this file keeps the
> closed jobs' narrative as it stood, and is not maintained.

# State — Wind's handover snapshot

**Lot 5 (E13–E15) ACTIVE since 2026-09-07 — E13 (`2e096d6`), E14
(`516180e`) and E15 (`46c1463`; the audit's evidence folder `92fa3b6`
before it) field-validated and committed locally
...
**Uncommitted in the working tree (2026-09-07):** `e2e/language-gate.mjs`
carries a comment-only edit recording the Chief Engineer's ratification of the
`docs/evidence/` exemption. The exemption itself shipped in `92fa3b6`; only its
wording, dated and attributed, is pending. It does not deserve a cold gate of
its own — **fold it into the next commit that runs a full gate anyway**, and
delete this paragraph when it lands.

**Lot 3 ACTIVE:** continue the remaining work listed above from the
[focused plan](PLAN-AUDIT-2026-09-LOT3.md). Preserve the approved early visual
verdict, same-mailbox generic repair, both protocol probes and recurrence subset.
...
✅ **[PLAN-RETOURS-15](PLAN-RETOURS-15.md)** (2026-09-04, one commit `2160fac`, field "All ok" zero KO, CI green run 33917364222, System **A117**) — **the first job sourced from the beta**: the organized Reception honors the panes setting (reverses E4/A99 — at 3 panes the standard grid, sections in the 400 px list column, a click reads IN the pane; 2/1 panes unchanged, V-D2; one `organizedPaneless` predicate; a read row never jumps under an open reading, the seam re-serves at the next serving). **D-51 PAID and closed** (decision D4): `MailServer::fetch_flags`, a bounded 500-UID `(UID FLAGS)` window on CONDSTORE-less servers, plus the review's catch — a LIGHT flags pass when the guarded poll skips (a flag-only change is invisible to STATUS without HIGHESTMODSEQ; window from the store's own UIDs, zero inventory, ADR 0017 held) and `flags_applied` bumps the UI generation; a live queued intent wins over the window (`refusee = 0`); stated limit: stale beyond the window. Fresh-eyes review ten findings, nine fixed, one skipped as stated limit (CONDSTORE upsert path's narrow mid-poll race, pre-existing). e2e trap paid: two specs now purge `wind-volets` — a stale 2 from an interrupted run silently relaid the suite at two panes. Tests mail-core 465 → 469, e2e 205, flaky 0. **The next release is 0.18.0 MINOR (D3)** — it carries this job AND audit wave 3 (its D5); CHANGELOG entry written at close. Closed the same day, just before it:

✅ **[PLAN-AUDIT-V3](PLAN-AUDIT-V3.md)** (2026-09-04, eleven commits `5acfdc6` → `081bd56`, two field passes, last CI 33893884411) — **audit wave 3, and with it the WHOLE audit of 2026-09-01 is delivered** (waves 0-3). One TLS stack — rustls + platform verifier, config built once per process (E1, ADR 0032, spike 9/9 live handshakes); `store.rs` 10,568 → 2,890 lines across six submodules, the pure decisions of `upsert_envelopes` extracted RED-first (E2); typed `CommandError` + `with_store` (open sites 93 → 21), one `poll_cycle` for the twins, `queue_send` packed, D-34 closed (E3); the poll policy in `mail_core::cycle` behind `CycleHooks`, the watcher free of `commands::` (E4, ADR 0033); the cadence shell-side — core `Cadence` decides, a 15 s thread drives the same commands, D-52 item 3 closed by the `ui_state` drafts revision (E5, ADR 0034, D4); the `MailServer` trait cleaned, `Folder.delimiter`, D-30 closed by the widened repair, `capabilities()` refused §2.6 (E6, ADR 0035); the front — `views.svelte.js` closing D-48 with 13 shell-side generation bumps, the 8 `__e2e*` seams compiled out of releases behind `VITE_E2E` with a make-release absence assert, `Editor.svelte` out of Compose, D-47's UI half (E7, D7 trimmed the letter: transport.js already was the one doorway, the big-file splits refused). Field: the D6 intermediate pass caught four findings all resolved same day — offline gained the red alert disc (A116), a recipient-less draft now MIRRORS to the server (Chief-Engineer ruling reversing a documented limit), the "resweep" exonerated as legitimate CONDSTORE movement and instrumented (`poll_reason` traces a masked folder + reason); final pass "All OK", zero KO. Review: ten findings, eight fixed in session. **D-51's reopening condition fired** (the Chief Engineer's account 3 is CONDSTORE-less — register note, call due at the next sync job). Register: D-30, D-34, D-48, D-52(3,8) closed; D-59 opened (attachment rank/filter tripwire). Kaizen: ~87 M input equivalents in one 9 h session, ~27 full gates, 0 KO at the final STOP 2 — figures on the closed plan. Behavior-neutral by contract: **no release owed** (D5 — the wave rides with the next feature release, which will be a PATCH-or-more per what joins it).

✅ **0.18.0 PUBLISHED on 2026-09-04** (D3 of PLAN-RETOURS-15 — it carries RETOURS-15 AND audit wave 3; release commit `b17f033`, bare tag, Latest; `verify-release.ps1`: **everything passes** — 5 assets, BOM-less `latest.json` 1,585 b on both platform keys, signatures == `.sig` and distinct, arm64 exe 200 / 5,953,379 b, x64 exe 200 / 6,729,457 b; minisign crypto not proven for lack of the tool on PATH, as at 0.16/0.17; **auto-update 0.17.0 → 0.18.0 proven on BOTH workstations the same day** — Chief-Engineer GO: "Auto update ok on both stations"). NB: the x64 workstation runs Smart App Control On — another favorable per-binary verdict; the SAC net's due proof (a VISIBLE failure under a real refusal) stays armed. Nothing more is owed on this version.

✅ **0.19.0 PUBLISHED on 2026-09-05/06** (the macOS debut, both mac families — release commit `171d1ec`, bare tag, Latest; the mac half run on the Air 2026-09-06 by `release-macos.sh` at `171d1ec`; `verify-release.ps1`: **36 PASS, 11 assets, 4 platform keys, 4 signatures pairwise distinct, both dmgs resolve whole**; minisign crypto NOT PROVEN, tool absent, as always). Three field findings at the first two-family mac release, all fixed the same day (`875d999`, `0f0f463`, CI green 33997162219 / 33997538007): an unauthenticated `gh` reported "no release" for a published one (the script now checks `gh auth status` first); a stray `npm install` on the Air (vite 8) left the tree dirty (guide rule: only `npm ci` on the Mac, recovery commands written); a `Wind` image left mounted plus the packager's `dmg.*` temp volume made `bundle_dmg.sh` fail AFTER the 5-min x64 build (the script now refuses over a mounted volume before building). **Field proofs still open**: the x64 INSTALL on the Air by the Chief Engineer (`Wind_0.19.0_x64.dmg`), the Apple Silicon INSTALL by the requesting tester (`Wind_0.19.0_aarch64.dmg` — the run proof of PLAN-APPLE-SILICON), the mac AUTO-UPDATE at the next release (ADR 0036), the Windows auto-update per channel as usual.

🔧 **Next subject**: **the beta feedback** — the Apple Silicon tester's install report goes first, in `/field`; then (MINOR, D7 of PLAN-MACOS — the macOS debut; CHANGELOG entry ready; order: Windows `make-release.ps1`, then `release-macos.sh` on the Air (both mac families, two key prompts), then `verify-release.ps1` expecting 11 assets/4 keys; the first mac release proves INSTALL on x64 by the Chief Engineer and on Apple Silicon by the requesting tester — the run proof of PLAN-APPLE-SILICON —, the second proves the update) — or **the beta feedback** (wave 1 open since 2026-08-31, five invitations out — a tester report goes first, in `/field`, on `main`; follow-up with the silent ones was due 2026-09-03). **The GitHub support ticket is RESOLVED (2026-09-04)**: support confirmed the purge, and the check agrees — `gh api repos/smonchamps/wind/commits/051bb01` answers `422 No commit found for SHA` (the API returns 422 for a short SHA, not the 404 this document predicted; same meaning, the object no longer resolves). Still awaited: the SAC net (an update refused for real — armed, not closable by us), the D-50 Microsoft-refresh confirmation (due ≈ 2026-12-01, 90 days from 2026-09-02). **The S4 measure is CLOSED (2026-09-04)**: workstation 1, from `wind.db`, 5/18/18/9/19/11 unknown senders per day over 2026-08-30 → 09-04, **~13/day, peak 19** (the honest query tests "no routing line whose epoch precedes the arrival" — a still-unrouted-today count undercounts at ~6/day because verdicts erase their senders); workstation 2: 4 on 2026-08-30, none after (a quiet workstation) — the Screener's sizing rests on workstation 1's figures. D-51 is being PAID in PLAN-RETOURS-15 (open). With the audit closed, the product PLAN's path is the beta itself.

Closed just before it, same day:
**[PLAN-ENGLISH-SWITCH](PLAN-ENGLISH-SWITCH.md)** (2026-09-04, 46 commits, nine field passes, CI green run 33856383445) — the whole repository switched from French to English in three days: code (E2-E5), e2e (E6), the living docs and ADRs under their English names (E7), the System in place with A115 (E8), the 53 archives frozen under an English banner (E9, debt D-58), the memory (E10). Language baseline 142 113 → ~1 985; what stays French is decided debt (D-55 persistence, D-56 shell text, D-57 illustrations, D-58 archives) plus the French catalogue delivered word for word. Kaizen: ~434 M input equivalents over 8 sessions, 12+ full gates, 0 KO at the five final STOP 2 passes — figures on the closed plan.


The E0-E10 delivery record of the switch, step by step → the closed plan; what follows is the historical narrative of the job as it ran:

**Job history: [PLAN-ENGLISH-SWITCH](PLAN-ENGLISH-SWITCH.md)** — switch all the code and all the documentation from French to English (CE command). Plan written and measured on 2026-09-02 (~375 000 words of prose, ~1 200 definitions, ~115 files to rename; layer by layer, one commit per layer, three new nets). **STOP 1 played on 2026-09-02, fourteen decisions settled** (D1 archives frozen, D3 French SQL kept → debt D-54, D4 English by default → next release MINOR, D9 normative documents read in full by the CE, D11 `BETA.fr.md` kept, D14 glossary validated by the CE), **GO from the CE on 2026-09-02** after the publication of 0.16.0 and 0.17.0 and the close-out of AUDIT-V2. **E0 delivered and validated on 2026-09-02** ([GLOSSARY.md](GLOSSARY.md) « Validé tel quel », `scripts/rename/`: 1 210 identifiers, 480 keys, 542 DOM ids derived from `tokens.csv`). **E1 delivered on 2026-09-02**: four nets proven by breaking them — the language ratchet (142 113 French markers at baseline, `e2e/language-baseline.json`, any rise = red), the IPC contract (`queue_send` invisible until then), markdown links, eslint `no-undef` (set-based: svelte-check rejected at 1 059 pre-existing errors); a 13-step gate. **E2 delivered on 2026-09-02**: scripts renamed and rewritten in English (`make-release.ps1`, `verify-release.ps1`, `run-wind.ps1`, `install-workstation.ps1`, `build-wind.mjs`, `measure-sessions.mjs`, `field.ps1`, `make-icon.ps1`), skills `/job` `/field` `/close` `/gate`, agent, CLAUDE.md, WORKFLOW.md, gate.ps1, hook, CI; STANDARD §2.8 amended — everything new is written in English from here on. **E3a delivered on 2026-09-02**: `mail-ical`, `mail-render`, `mail-smtp`, `mail-auth` in English (3 476 lines, 96 tests green, mail-ical public API renamed with its two dependents). **E3b delivered on 2026-09-02**: `mail-imap` in English (3 398 lines, 79 tests green, `Veille`/`veiller` → `Watch`/`watch` with the shell's watcher updated). **E3c delivered on 2026-09-02, in three commits**: `store.rs` (10 504 lines, 140 tests — split in seven chunks translated in parallel by Sonnet agents against a fixed rename table, reassembled, dependents updated by a string-literal-aware whole-identifier pass), then `sync`/`thread`/`search`/`backfill` (4 822 lines, 120 tests), then the rest of the crate and its 13 examples renamed (`correspondants.rs` → `contacts.rs`, `bench_*`, `diag_*`, `seed_arrival`). Every Rust crate is now in English; `mail-core` keeps 124 French markers, all deliberate (`lang:fr`: notification texts, quoting and forward labels composed into bodies, the size units `o`/`Ko`/`Mo` the e2e specs assert, the French fixtures that ARE the test). Baseline 134 436 → ~110 100. Two traps caught by the nets and fixed the same hour: the mechanical rename reached the shell — 14 Tauri command names (IPC contract) and the serialized `dernier_epoch`/`dernier_objet` payload fields the UI sorts on (e2e `feedback-14`) — both reverted, they belong to E4. **E4 delivered on 2026-09-03**: the shell (`apps/desktop/src`, 8 552 lines) in English — the 36 command names of GLOSSARY §5.3 renamed together with the UI `appel()` calls, the specs' `invoke()` calls and the two e2e tools that name commands; `veilleur.rs` → `watcher.rs`, `demenagement.rs` → `relocation.rs`; `hors_pompe` → `off_pump` with the main-thread guard's literal. Kept for E5 on purpose, because they are the IPC contract with the UI: the command PARAMETER names (JSON keys) and the serialized payload FIELDS; the two native dialogs stay French (`lang:fr`). Shell: 33 French markers, all deliberate. Baseline → 102 688. **Field validated on 2026-09-03 (E3c + E4, no finding).** **E5 STOP 1 played on 2026-09-03** (D15 DOM contract at E5d, D16 value vocabularies mapped at the shell boundary in `wire.rs` — the database keeps the French value, the wire carries the English one —, D17 shell-composed text kept French, debt D-56). **E5a delivered on 2026-09-03**: the IPC keys (18 parameters, ~45 fields) and the five value vocabularies in English on the wire, `--rep-*` → `--mk-*` (A110); `d384724`, CI green 33742728494, **field validated on 2026-09-03, no finding**. **E5b delivered on 2026-09-03**: the UI in English — 38 files renamed, ~850 identifiers (the E0 dictionary plus 282 definitions it had missed, found by seven read-only Sonnet agents), the 518 catalogue keys and the `{placeholders}` of the catalogue values, ~1 000 comment blocks translated by eight Sonnet agents under a mechanical oracle (files stripped of comments byte-identical to the snapshot: 0 code difference); **D4 applied** (English is the reference: `lib/language.js`, `text.svelte.js` falls back to EN, `Lang::from_pref` defaults to `En`, ADR 0016 amended, A111 — **the next release is MINOR**); two persisted UI preferences migrate at read without a reset (`wind-largeurs` key `liste` → `list`, `wind-espacement` `faible|moyen|eleve` → `low|medium|high`); the applier is committed (`scripts/rename/apply-ui.mjs`, a tokenizer with a `--report` mode, GLOSSARY §6). Baseline 102 638 → 87 926; the UI keeps 96 markers, all deliberate. Fresh-eyes review: ten findings, eight fixed (a legacy map the applier had rewritten, a duplicate catalogue key, two nets that matched nothing since E5a, the chip tone classes, D4 now proven in a real launch, a placeholders net), two deferred to E5d (`line` → `row`, the handle test id). Lesson: the catalogue-value `{placeholders}` are a bridge too (a param key renamed on one side rendered empty names, organizers and dates — nine specs caught it); an e2e wave played while passes are still running is worth nothing. Full gate green in 167 s (198 e2e, flaky 0); commit `59c6ee1`, CI green 33759685493; **field validated on 2026-09-03, no finding** (ten steps OK, trace clean). **E5c delivered on 2026-09-03**: `--brand`, `--r-control`, `--r-tile`, `--tile`/`--tileInk` in the components, `system.css` (renamed), the System (A112, contract table cells included) and the three nets; the four sort glyphs renamed `sort_*`; nothing visible changes; full gate green in 145 s (198 e2e, flaky 0); commit `29c6a68`, CI green 33765915506, **field validated on 2026-09-03, no finding**. **E5d delivered on 2026-09-03**: the DOM contract in English from one table — `dom.csv` completed 539 → 654 rows by `scripts/rename/derive-dom.mjs` (the E0 inventory had missed 124 classes and the `data-*` attribute names), 49 words entered in `tokens.csv` (D20 + five listed in the plan's annex for STOP 2), 22 E0 rows corrected; the applier `scripts/rename/apply-dom.mjs` (RED then GREEN on fixtures, `e2e/apply-dom.test.mjs`) applied to 67 files in one run — the components, `system.css`, 30 specs, 5 e2e tools; `line` → `row` for the list row (D18, `listRow` snippet, `thread.row`); the two test-id collisions split (D19: `write`/`compose`, `desk-continue`/`onboarding-continue`); the 12 `data-*` names (D21: `data-hue`, `data-category`…); the eight seams (`__e2eHold`, `__e2eLog`, `__e2eRelease`…); System A113. Seven spec-side reds on the first e2e wave (selector forms the pass did not reach — each a rule now), ten review findings fixed (one field-visible: the cancelled-invitation chip's alert ink), a permanent net added (`e2e/dom-contract.test.mjs`: every id a spec selects is rendered by the UI). Full gate green in 150 s (198 e2e, flaky 0); baseline 87 925 → 87 900; commit `2c30cea`, CI green 33781288186, **field validated on 2026-09-03, no finding** (ten screens, both themes; the five words added after D20 validated). **E6 STOP 1 played on 2026-09-03** (D22 the suite runs in English — the D4 default, D23 the 201 titles translated, D24 the e2e README rewritten, D25 the living docs' path pointers updated names-only, D26 the four names the glossary lacked). **E6a delivered on 2026-09-03**: the 30 specs and 13 tools renamed (`redesign-screen02.spec.js`, `contrast.mjs`, `system-coherence.mjs`, `main-thread-guard.mjs`, `tokens.mjs`, `freeze-probe.py`, `measure-*`…), identifiers, comments and titles in English (the applier `scripts/rename/apply-e2e.mjs` on the shared scanner `scripts/rename/lib.mjs`, ten Sonnet agents under a token-level oracle), the 21 dependents and living docs re-pointed, System A114; baseline 87 900 → 77 283 (the layer 10 981 → 364, all E6b anchors); gate green 209 s (196 e2e, 2 flaky), review twelve candidates / ten fixed (three `file:line` pointers had drifted by one line — a comment pass moves lines). **E6b delivered on 2026-09-03**: the suite launches in English by default (D22), ~200 anchors rewritten from the catalogues, 95 fixture lines `lang:fr`, two French-only tests added to `redesign-language.spec.js` (the R3 short name, the plural), `capture-onboarding.mjs` pinned French; the e2e layer at **0 French markers**, baseline → 76 919; one field-visible symptom found by the wave: the compose weight reads `2.8 Mo / 25 MB` (shell unit vs catalogue limit, debt D-56). **Field (STOP 2) on 2026-09-03: E6 validated by the Chief Engineer, no finding** (benches under their new names, `freeze-probe` 0 freeze > 150 ms, `measure-v2` page p50 15.1 ms, gate green 157 s); **D27 applied** (`MEASURE_DB`, `MEASURE_ACCOUNTS`, `MEASURE_REUSE`, `MEASURE_NO_ACTIVITY`), **D28 applied** (the French sweep: three more tests in `redesign-language.spec.js`; rule: every screenshot in the language the user chose → debt D-57, the onboarding illustrations at the next onboarding job). CI green 33802706071 (E6a) and 33806065399 (E6b). **E7 delivered on 2026-09-04, fourteen commits** (`625f8d4` → `57b83ed`): the living documents in English — README, STANDARD (structure §0-§10 frozen, heading oracle held; nine stale bench/diag pointers and the -DocsOnly flag fixed in passing), PLAN.md, BETA.md (D11: `BETA.fr.md` kept word for word), AUDIT-2026-09-01, CHANGELOG (D13: all 30 entries, the `## [x.y.z]` lines byte-identical — make-release.ps1 still parses), the v3 annotations, the icons note; the decided renames applied with every referrer swept in the same commit (GLOSSARY §5.1-§5.2): `ETAT` → `STATE`, `DETTE` → `DEBT` (D-n multiset identical), `PASSATION` → `HANDOVER`, the 31 ADRs under their English slugs, `PLAN-BASCULE-ANGLAIS` → `PLAN-ENGLISH-SWITCH`; the 23 closed PLAN-*.md moved to `docs/archives/` (their place — D1 freeze banner due at E9), PLAN-BETA and PLAN-KAIZEN-CLAUDE translated; WORKFLOW.md was already English (E2) — its slot was a no-op. Repaired in passing: the D22-D26 decision records sat jammed on line 1 of the plan since the E6 STOP 1 edit, now in their table rows. Method: 25 Sonnet agents under structure oracles (headings, D-n, `## [` lines), docs-links and the ratchet green at each of the fourteen commits. Baseline 76 919 → 19 633 (what remains: the frozen archives are excluded; the residue is the deliberate `lang:fr` code markers, the French catalogue values and GLOSSARY's own mapping rows). **Field (STOP 2) on 2026-09-04: E7 validated by the CE, no finding** — D9 reread of STANDARD, WORKFLOW and the 31 ADRs done; T6 proven twice (branch refusal in English, then the missing `## [0.18.0]` entry read from the translated CHANGELOG); T7 by the push itself (pre-push gate, CI green 33812777584). **E8 delivered on 2026-09-04**: the System in English, in place (D8, one commit) — nine Sonnet chunks under four oracles (tag skeleton, the 68 token cells, the 91 figcaption glyphs, the A-numbers, all byte-identical), the mockup UI strings quoting `catalog.en.js`, sample data kept as fixtures; the file renamed `system.dc.html` (GLOSSARY §5.1), `data-jeton` → `data-token` with `system-coherence.mjs` in the same commit, amendment **A115** (129 journal rows); the A114 rename pairs restored (the translation had erased their French left sides); `architecture/index.html` translated with its stale bench/script pointers fixed and its SVG aria-labels in English. system-coherence, contrast (440 pairs) and docs-links green; baseline 19 634 → 1 985 (the residue: `lang:fr` code markers, the French catalogue, GLOSSARY mapping rows, and the scanner counting the abbreviation "CE" as French). **Field (STOP 2) on 2026-09-04: E8 validated by the CE, no finding** — visual review of the System in both document themes, D9 full reread, the architecture map skimmed. **E9 delivered on 2026-09-04**: the 53 archives frozen under an English banner (52 markdown heads with their closing date where recorded, one visible strip in `systeme.v1.dc.html`) — decision D1, written to the register as **debt D-58** (the plan predicted D-55, a number since taken); the ratchet and the links net already excluded the directory. **Field (STOP 2) on 2026-09-04: E9 validated by the CE, no finding** (banners spot-checked, v1 System strip seen). Then E10 (memory, close-out).
...
(`docs/design/system.dc.html`, ADR 0026, journal A79, the old one archived
...
- **Expensive queries from periodic probes** (PLAN-GELS D4): off
  ...
  register **D-8** of [DEBT.md](DEBT.md), figures and leads inside.
