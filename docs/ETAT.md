...
2026-09-01** : `freeze-probe.py` 60 s sur décor Clarity, clics répétés
...
`multi-select:173` ×3). **0.16.0 PUBLIÉE le 2026-09-02** (`343c8d0`,
...
`.ps1` ; et `thread-bars:25` attend la carte avant de la lire.
...
- **zéro cadratin** dans les textes montrés (D4, filet
  `catalogs.test.mjs` prouvé en le cassant) ;
...
son parseur centralisé dans `tokens.mjs` (`lireReperes` — les deux
...
(redesign-panes:86, scrim de composition qui intercepte le clic —
...
le titre et l'entête de `redesign-feedback-8.spec.js` disaient « quatre
...
(`e2e/capture-onboarding.mjs`, rejouable), vignettes de thème dans la
...
bout-en-bout p50 2 408 → **17 ms** (banc `measure-scroll.mjs`,
...
d'avant conservée — tenu par la gate `main-thread-guard.mjs` et le
budget « aucun gel de pompe > 150 ms » (`freeze-probe.py`). Au passage, le
