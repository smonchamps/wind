...
🔧 **Chantier en cours : [PLAN-BASCULE-ANGLAIS](PLAN-BASCULE-ANGLAIS.md)** — basculer tout le code et toute la documentation du français vers l'anglais (commande CE). Plan rédigé et mesuré le 2026-09-02 (~375 000 mots de prose, ~1 200 définitions, ~115 fichiers à renommer ; par couche, un commit par couche, trois filets neufs). **STOP 1 joué le 2026-09-02, quatorze décisions tranchées** (D1 archives gelées, D3 SQL français gardé → dette D-54, D4 anglais par défaut → prochaine release MINEURE, D9 normatifs relus en entier par le CE, D11 `BETA.fr.md` conservé, D14 glossaire validé CE), **GO CE le 2026-09-02** après la publication de 0.16.0 et 0.17.0 et le solde d'AUDIT-V2. **E0 livré et validé le 2026-09-02** ([GLOSSARY.md](GLOSSARY.md) « Validé tel quel », `scripts/rename/` : 1 210 identifiants, 480 clés, 542 ids DOM dérivés de `tokens.csv`). **E1 livré le 2026-09-02** : quatre filets prouvés en les cassant — cliquet de langue (142 113 marqueurs français à la base, `e2e/language-baseline.json`, toute hausse = rouge), contrat IPC (`queue_send` invisible jusque-là), liens markdown, eslint `no-undef` (set-based : svelte-check rejeté sur 1 059 erreurs préexistantes) ; gate à 13 étapes. **E2 delivered on 2026-09-02**: scripts renamed and rewritten in English (`make-release.ps1`, `verify-release.ps1`, `run-wind.ps1`, `install-workstation.ps1`, `build-wind.mjs`, `measure-sessions.mjs`, `field.ps1`, `make-icon.ps1`), skills `/job` `/field` `/close` `/gate`, agent, CLAUDE.md, WORKFLOW.md, gate.ps1, hook, CI; STANDARD §2.8 amended — everything new is written in English from here on. **E3a delivered on 2026-09-02**: `mail-ical`, `mail-render`, `mail-smtp`, `mail-auth` in English (3 476 lines, 96 tests green, mail-ical public API renamed with its two dependents). **E3b delivered on 2026-09-02**: `mail-imap` in English (3 398 lines, 79 tests green, `Veille`/`veiller` → `Watch`/`watch` with the shell's watcher updated). **Session closed here at 75 % of the context window (kaizen T2) — next session: E3c, `mail-core`** (~24 000 lines, `store.rs` 10 504, `commands.rs` of the shell is E4). Handover for that session, in order: (1) read this paragraph, GLOSSARY.md §1-§4 and `scripts/rename/dictionary.csv` (rows `layer=rust`, file column) — the words are decided, do not re-decide them; (2) `mail-core` in three commits: `store.rs` alone, then `sync.rs` + `thread.rs` + `search.rs` + `backfill.rs`, then the rest (`nav.rs`, `drafts.rs`, `outbox.rs`, `echo.rs`, `compose.rs`, `correspondants.rs` → `contacts.rs`, `invitation.rs`, `body.rs`, `address.rs`, `action.rs`, `attachment.rs`, `crash.rs`, `envelope.rs`, `error.rs`, `imap_quoted.rs`, `notify.rs`, `remote.rs`, `transport.rs`, `test_support.rs`, the 13 `examples/` renamed per GLOSSARY §5.1); (3) every public item renamed must have its dependents (`apps/desktop/src`, `mail-imap`, `mail-smtp`) updated in the SAME commit — `cargo build --workspace` is the oracle, `grep -rn 'mail_core::'` finds them; (4) the SQL text, the `prefs` keys and the stable strings (`"accepte"`, `"sans_reponse"`, `"request"`…) do NOT change (D3) — only the Rust names around them; (5) test names are English sentences, by hand (`scripts/rename/test-names.txt`); (6) the `#[error]` messages and the `trace()` lines go English (D5); a line that must stay French carries `lang:fr`; (7) after each commit: `node e2e/language-gate.mjs --update`, then the full gate, then commit in English, push and CI watch **as a background tool call** (never a trailing `&` — it backgrounds the whole chain and hides the push); (8) E3 rate measured: four small crates (3 476 lines) in one hour, mail-imap (3 398) in one hour — plan ~7 hours for mail-core. Then E4 (shell + IPC, `scripts/rename/dictionary.csv` and GLOSSARY §5.3), E5 (UI + catalogue keys `keys.csv` + DOM `dom.csv`), E6 (e2e/scripts), E7-E10 (docs, System, archives, memory).
