...
const ram = async (phase) => {
  const jsHeapMb = await heap();
  ...
  record({ phase, mb, jsHeapMb, raw: total, detail });
};
...
try {
  ...
  await ram('feed-page-1');
  // Proof of rendering: a body must be VISIBLE in each option.
  await page.screenshot({ path: path.join(rawDir, `${option}-${ko}k-run${run}-page1.png`) });
  ...
  // The routed decor holds 160 cards (16 senders x 10): the loop ends
  // when a scroll to the bottom grows nothing within 12 s (4 s in runs 1-2 — too short at 300 KB).
  for (let i = 0; i < 20; i += 1) {
    const before = count;
    const s = performance.now();
    await cards.last().scrollIntoViewIfNeeded();
    for (let w = 0; w < 120 && count === before; w += 1) {
      await sleep(100);
  ...
  await ram('feed-10-pages');
  await page.screenshot({ path: path.join(rawDir, `${option}-${ko}k-run${run}-10pages.png`) });

  await page.locator('[data-testid="nav-folder"][data-category="inbox"]').click();
  await sleep(8000);
  await ram('back-inbox');
  await sleep(25000);
  await ram('back-inbox-25s');
  // Diagnostic beyond the three options: a FORCED GC in the renderer
  // (CDP) — what remains after it is retained, what leaves was garbage
  // Chromium had simply not collected yet.
  try {
    const cdp = await page.context().newCDPSession(page);
    await cdp.send('HeapProfiler.collectGarbage');
    await sleep(5000);
    await ram('back-inbox-after-gc');
  } catch (err) {
    record({ phase: 'back-inbox-after-gc', error: String(err) });
  }
} finally {
