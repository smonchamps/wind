...
//
// Writes apps/desktop/ui-v2/src/assets/accueil/<lang>/disposition-{3,2,1}.png (en, fr)
...
// Lot 5 E15d (D-57): one capture set per language, under
// assets/accueil/<lang>/ — the onboarding shows the interface the user
// is about to use, in their language.
for (const lang of ['en', 'fr']) {
  const target = path.join(folder, lang);
  mkdirSync(target, { recursive: true });
  const { app, browser, page } = await launchAppV2({ lang });
  try {
    for (const panes of [3, 2, 1]) {
      ...
      await page.screenshot({
        path: path.join(target, `disposition-${panes}.png`),
