...
const poserCran = async (niveau) => {
  ...
  await page.locator('[data-testid="affichage-espacement"]').selectOption(niveau);
...
const sujetAuSommet = () => page.evaluate(() => {
  ...
  let gagnant = null;
  let ecart = Infinity;
  for (const l of cadre.querySelectorAll('[data-testid="ligne"]')) {
    const d = Math.abs(l.getBoundingClientRect().top - haut);
    if (d < ecart) { ecart = d; gagnant = l; }
  }
  return gagnant?.querySelector('.objet')?.textContent?.trim() ?? null;
});
...
test('la barre de défilement dit la hauteur RÉELLE des rangées', async () => {
  ...
  for (const cran of CRANS) {
    ...
    const mesure = await page.evaluate(() => {
      const cadre = document.querySelector('.cadre');
      const lignes = [...cadre.querySelectorAll('[data-testid="ligne"]')];
      const nue = lignes.find((l) => !l.querySelector('.puces'));
      const porteuse = lignes.find((l) => l.querySelector('.puces'));
      return {
...
test('bascule à chaud AVEC une conversation épinglée — le chemin du défaut', async () => {
  ...
  await expect(page.locator('[data-testid="epingles"]')).toBeVisible();
...
test('hors du flot fenêtré, changer de cran ne déplace pas la liste', async () => {
  ...
  await page.locator('[data-testid="champ-recherche"]').fill('message');
  ...
  await page.evaluate(() => { document.querySelector('.cadre').scrollTop = 300; });
...
test('la fenêtre se recalcule quand le cadre grandit (décision CE D3)', async () => {
  ...
  const compter = () => page.locator('[data-testid="ligne"]').count();

  await page.evaluate(() => { document.querySelector('.cadre').style.flex = '0 0 300px'; });
  await page.waitForTimeout(200);
  const court = await compter();

  await page.evaluate(() => { document.querySelector('.cadre').style.flex = '0 0 1400px'; });
  ...
  await page.evaluate(() => { document.querySelector('.cadre').style.flex = ''; });
  await page.waitForTimeout(200);
});
