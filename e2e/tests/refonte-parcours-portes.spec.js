...
test.describe('décor v1 : un compte, 200 messages', () => {
  ...
  test('brouillon : Échap conserve, le dossier Brouillons restitue intact', async () => {
    ...
    const ligne = page.locator('[data-testid="row-draft"]', { hasText: 'Brouillon E2E' });
  ...
  test('mise en forme : le gras s’applique, le brouillon RICHE survit à la reprise (PLAN-COMPOSITION-HTML)', async () => {
    ...
    await page.locator('[data-testid="nav-folder"][data-category="drafts"]').click();
    const ligne = page.locator('[data-testid="row-draft"]', {
  ...
  test('pièces jointes : la ligne porte ses puces, le volet de lecture dit les pièces (PLAN-RETOURS-V3 R1)', async () => {
    ...
    const porteur = page.locator('[data-testid="row"]', { hasText: 'n°190' }).first();
    await expect(porteur.locator('[data-testid="chips-row"]')).toContainText('2 messages');
    await expect(porteur.locator('[data-testid="chips-row"]')).toContainText('1 fichier');
    // Une ligne SANS puces n'a pas de rang du tout — elle est plus
    // basse : deux gabarits, la mécanique de fenêtrage d'avant A29.
    const nue = page.locator('[data-testid="row"]', { hasText: 'n°198' }).first();
    await expect(nue.locator('[data-testid="chips-row"]')).toHaveCount(0);
  ...
  test('recherche : « / » focalise, les résultats servent, archiver en retire (régression #4)', async () => {
    ...
    const archive = await resultats.first().locator('.subject').textContent();
  ...
  test('brouillon : le supprimer depuis la composition, avec confirmation (PLAN-RETOURS-3 R3)', async () => {
    ...
    await page.locator('[data-testid="nav-folder"][data-category="drafts"]').click();
    const ligne = page.locator('[data-testid="row-draft"]', { hasText: 'Brouillon a jeter' });
