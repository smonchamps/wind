...
test("R4 : l'entête du composeur porte le fond du pied de page de Wind (A66)", async () => {
  ...
  const tete = await fond(page.locator('[data-testid="compose"] .head'));
  const pied = await fond(page.locator('[data-testid="status"]'));
...
test('R1 : la signature se pose aux Réglages, paraît au nouveau message — et fermer sans frappe ne sème rien', async () => {
  ...
  const editeur = page.locator('[data-testid="signature-editor"]').first();
  ...
  const de2 = page.locator('select[data-testid="compose-from"]');
...
test("R2 : un envoi programmé attend son heure, se dit, et s'annule en brouillon (D1/D2)", async () => {
  ...
  const fente = page.locator('[data-testid="slot-notice"]');
...
test("R3 : « Important » se marque, suit le brouillon repris, et part au journal", async () => {
  await page.locator('[data-testid="write"]').click();
  const marque = page.locator('[data-testid="compose-important"]');
  ...
  // dossier Brouillons a SON testid : `row-draft`, pas `row`.)
  await page
