...
//   remplis  solid paths in ink (currentColor)
//   r        RESERVED for the set, used nowhere (A53/A60/A62)
//   repere   glyph of the set dedicated to account markers (A74)
...
export const SET = {
  ...
  mark_email_unread: { d:['M3 8h14v10H3z','M3 8l7 5 7-5'], dot:[19,6,3.4] },
  ...
  schedule_send: { d:['M9 3a6 6 0 1 0 0 12 6 6 0 1 0 0-12','M9 6v3l2 2'], filled:['M14 15l8 4-8 4 2-4z'] },
  ...
  // The beta feedback (PLAN-RETOURS-11 R3): speech bubble with tail +
  // two lines of text. `forum` keeps “conversation” (the thread chip).
  ...
  format_color_text: { d:['M6 15l6-11 6 11','M9 11h6'], bar:'M4 19h16' },
  ...
  flight: { d:['M11 3h2l1 8 7 3v2l-7-1-1 5 3 2v1l-4-1-4 1v-1l3-2-1-5-7 1v-2l7-3z'], marker:true },
  home: { d:['M3 11l9-7 9 7','M6 10v10h12V10'], marker:true },
  music_note: { d:['M11 18V6','M11 6l5 3'], dots:[[8,18,3]], marker:true },
  pets: { d:['M12 21a4 4 0 0 1-4-4c0-3 8-3 8 0a4 4 0 0 1-4 4z'], dots:[[4,10,2],[9,6,2],[15,6,2],[20,10,2]], marker:true },
  school: { d:['M2 9l10-4 10 4-10 4z','M6 12v5c0 2 12 2 12 0v-5'], marker:true },
  shopping_bag: { d:['M5 7h14v13H5z','M9 7V5a3 3 0 0 1 6 0v2'], marker:true },
  sports_esports: { d:['M4 16l2-7h12l2 7a2 2 0 0 1-3 1l-2-2H9l-2 2a2 2 0 0 1-3-1z'], filled:['M7 11h2v2H7z','M15 11h2v2h-2z'], marker:true },
  star: { d:['M12 3l3 6 6 1-4 5 1 6-6-3-6 3 1-6-4-5 6-1z'], marker:true },
  work: { d:['M3 8h18v11H3z','M9 8V5h6v3'], marker:true },
  volunteer_activism: { d:['M12 9S9 6 7 6a3 3 0 0 0 0 6l5 4','M12 9s3-3 5-3a3 3 0 0 1 0 6l-5 4','M3 14v7h4l6 2 8-4v-2h-8'] },
  // Organized mode (PLAN-MODE-ORGANISE, decision D9): the prototype's
  // drawings come in as is — board spikes/mode-organise/.
  screener: { d:['M12 4a3 3 0 1 0 0 6 3 3 0 1 0 0-6','M5 20v-1a7 7 0 0 1 14 0v1','M9.5 13.5v3l2.5-1.5z','M14.5 13.5v3l-2.5-1.5z'] },
  // RETOURS-13 field finding: the feed becomes a NEWSSTAND —
  // scalloped awning (arches), booth, counter. CE verdict facing the
  // board of 7 (variant B), replaces the prototype's drawing.
  ...
  // E2: the ellipsis of the Screener's minis ⋯ — three
  // solid discs in ink, the Material shape redrawn to the
  // set's grammar (the prototype already carried them).
  more_horiz: { dots:[[5,12,2],[12,12,2],[19,12,2]] },
};

// The brand: outside the inventory — the icon document's glyph,
// verbatim: stroke 2.3 and half-disc r 3.25 tangent to the top edge.
export const BRAND = { d:['M4 8h16v9H4z'], stroke:2.3, flap:'M8.75 9.15A3.25 3.25 0 0 0 15.25 9.15Z' };

// The RESERVED glyphs and the dedicated marker set, derived from the
// catalogue — never a second list.
export const RESERVES = Object.entries(SET).filter(([, g]) => g.r).map(([n]) => n);
export const MARKERS = Object.entries(SET).filter(([, g]) => g.marker).map(([n]) => n);
